...
class PdfStructure
{
    ...
    private function processPiecePart($piecePart)
    {
        if ($piecePart instanceof PdfText) {
            ...
            if ($piecePart instanceof PdfIndent) {
                $this->processIdentPart($piecePart);
            } else {
                if ($piecePart instanceof PdfImage) {
                    $this->processImagePart($piecePart);
                } else {
                    if ($piecePart instanceof PdfPosition) {
                        $this->processPositionPart($piecePart);
                    } else {
                        if ($piecePart instanceof PdfNewline) {
    ...
    {
        $this->currentPage->addLine(new ImageLine($this->images[$image->getName()]['id'],
                                                  $image->getWidth(),
                                                  $image->getHeight(),
                                                  $this->x,
    ...
    private function writeText(string $text, PdfFont $font)
    {
        ...
        for ($i = 0; $i < $n; $i++) {
            ...
            if ($length > $this->width) {
                $this->currentPage->addLine(new TextLine(substr($text, $start, $i - $start - 1),
                                                         $this->x,
                                                         $this->y,
        ...
        $this->currentPage->addLine(new TextLine(substr($text, $start, $n - $start),
                                                 $this->x,
                                                 $this->y,
                                                 $this->fonts[$font->getName()]['id'],
                                                 $font->getSize()));
    ...
    private function addImagesToResource(Resource $resource)
    {
        foreach ($this->images as $imageName => $imageData) {
            ...
            $imageElement = new Image($image->getWidth(),
                                      $image->getHeight(),
    ...
    private function createTrailer()
    {
        ...
        $this->trailer = new Trailer(new Numeric($this->xrefTable->getCount()),
                                     $root,
